...
<h1>Department Form</h1>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="Det_Id" name="Det_Id"><br><br>

        <label for="Department_Name">Department_Name:</label>
        <input type="text" id="dpn" name="dpn" required><br><br>

        <label for="Department_Location">Department_Location:</label>
        <input type="text" id="dpl" name="dpl" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO department (Department_Id, Department_Name, Department_Location) VALUES ( ?, ?, ?)");
        $stmt->bind_param("sss", $Department_Id, $Department_Name, $Department_Location);

        // Set parameters from POST data with validation (optional)
        $Department_Id = ($_POST['Det_Id']); // Ensure integer for ID
        $Department_Name = ($_POST['dpn']); 
        $Department_Location = ($_POST['dpl']); 
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of department</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of department</h2></center>
    <table border="5">
        <tr>
            <th>Department_Id</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $did = $row['Department_Id']; // Fetch the Financial_info_id
                echo "<tr>
                    <td>" . $row['Department_Id'] . "</td>
                    <td>" . $row['Department_Name'] . "</td>
                    <td>" . $row['Department_Location'] . "</td>
                    <td><a style='padding:4px' href='delete_department.php?Department_Id=$did'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_department.php?Department_Id=$did'>Update</a></td> 
                </tr>";
            }
        } else {
